"""Live-dashboard + durable audit-trail event emitter for the orchestrator
itself. Same shape as each agent's src/events.py, writing to the same
shared dashboard/events.db - only the relative path depth differs since
this file has no src/ parent.
"""
import json
import os
import sqlite3
...
DB_PATH = Path(__file__).resolve().parents[1] / "dashboard" / "events.db"
