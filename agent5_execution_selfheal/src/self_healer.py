"""Self-heal: when a Playwright locator breaks, ask the LLM for a
replacement selector, patch the script on a new branch, verify the fix
actually works, and (only if it does) push + open a follow-up PR so the
fix stays traceable through Git rather than silently patching production
code.
"""
import re
from pathlib import Path

from src import events, git_ops, heal_prompt_builder, llm_client, runner
...
def heal(repo_path, script_relpath, target_page, broken_selector, output_dir):
    ...
    if not is_safe_selector(new_selector):
        events.emit(AGENT, "error",
                     f"AI-suggested selector failed safety validation, refusing to apply it: {new_selector!r}")
        return {
